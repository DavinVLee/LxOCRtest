...
@interface LxCameraView ()<AVCaptureMetadataOutputObjectsDelegate,UIAlertViewDelegate>
...
@property (strong, nonatomic) CALayer *limitCubeLayer;
...
//    [self addSubview:_flashButton];
    ...
    self.limitCubeLayer = [CALayer layer];
    self.limitCubeLayer.frame = CGRectMake((kScreenWidth - 290)/2.f, (kScreenHeight - 72)/2.f - 30, 290, 36.f/ 136.f * 290.f);
    self.limitCubeLayer.borderColor = [UIColor greenColor].CGColor;
    self.limitCubeLayer.borderWidth = 1;
    [self.layer addSublayer:self.limitCubeLayer];
    
    [self focusAtPoint:CGPointMake(self.limitCubeLayer.position.x, self.limitCubeLayer.position.y)];
...
- (void) shutterCamera
{
    ...
    [self.ImageOutPut captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
        ...
        UIImage *rotationImage = [self RotateImageBackCamera:self.image];
        CGRect rect = CGRectMake(CGRectGetMinX(self.limitCubeLayer.frame) / kScreenWidth * self.image.size.width * self.image.scale,
                                 CGRectGetMinY(self.limitCubeLayer.frame) / kScreenHeight * self.image.size.height * self.image.scale,
                                 CGRectGetWidth(self.limitCubeLayer.frame)/kScreenWidth* self.image.size.width * self.image.scale,
                                 CGRectGetHeight(self.limitCubeLayer.frame)/kScreenHeight *self.image.size.height*self.image.scale);
           CGImageRef imageRef = CGImageCreateWithImageInRect([rotationImage CGImage], rect);
        UIImage *image = [UIImage imageWithCGImage:imageRef scale:self.image.scale orientation:UIImageOrientationUp];
        ...
        [self insertSubview:_imageView belowSubview:_PhotoButton];
